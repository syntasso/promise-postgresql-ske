...
.PHONY: help build-workflow load-workflow push-workflow test system-test
...
test: ## Run kratix pipeline tests (no cluster required)
	$(MAKE) -C $(WORKFLOW_DIR) test

system-test: ## Run Ginkgo system tests against a live cluster
	$(MAKE) -C $(WORKFLOW_DIR) system-test WORKER_CONTEXT=$(WORKER_CONTEXT)
